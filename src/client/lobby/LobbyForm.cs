...
namespace Client
{
    public class LobbyForm : Form
    {
        ...
        public LobbyForm() : this("Người chơi") { }

        // Constructor nhận tên người chơi từ LoginForm
        public LobbyForm(string username)
        {
            ...
            // Dữ liệu ban đầu với tên người dùng thực tế
            lvPlayers.Items.Add(new ListViewItem($"🐺 {currentUsername} (Bạn)"));
            ...
            btnReady.Click += BtnReady_Click; // Gắn sự kiện Click
            ...
            // Đảm bảo tắt form này là tắt luôn toàn bộ ứng dụng
        ...
        private void BtnReady_Click(object? sender, EventArgs e)
        {
            ...
            if (isReady)
            {
                ...
                btnReady.BackColor = Color.FromArgb(160, 30, 120, 60); // Đổi sang màu xanh lá mờ
            }
            else
            {
                btnReady.Text = "SẴN SÀNG";
                btnReady.BackColor = Color.FromArgb(120, 20, 50, 60); // Trở về màu cũ
            }
        }

        // Hàm cập nhật danh sách người chơi từ Server
        public void UpdatePlayerList(string[] players)
        {
            if (lvPlayers.InvokeRequired) 
            { 
                lvPlayers.Invoke(new Action(() => UpdatePlayerList(players))); 
                return; 
            }
            ...
            {
                lvPlayers.Items.Add(new ListViewItem(p));
            }
            lblPlayerCount.Text = $"Số lượng: {players.Length}/10";
        }
    }
}